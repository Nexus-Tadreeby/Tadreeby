// src/components/pages/company-admin/Trainers.jsx
...
export default function Trainers() {
    ...
    const fetchTrainers = async () => {
        ...
        setError("");
        try {
            const response = await companyAdminAPI.getTrainers(1, 10);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await companyAdminAPI.deleteTrainer(id);
            await fetchTrainers();
        } catch (err) {
                    ...
                    <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="text-[11px] text-[#7B8497]">
                                Manage your company's trainers
                            </p>
                        </div>
                        <Button
                            variant="blue"
                            onClick={() => navigate("/company/admin/trainers/create")}
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <h4 className="text-[14px] font-extrabold text-[#172033]">
                                                    {trainer.user?.firstName || trainer.firstName} {trainer.user?.lastName || trainer.lastName}
                                                </h4>
                                            ...
                                            <span
                                                className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${trainer.user?.isActive !== false
                                                        ? "bg-[#EAF9EF] text-[#22C55E]"
                                                        : "bg-[#FEF0F0] text-[#EF4444]"
                                                    }`}
                                            >
                                                {trainer.user?.isActive !== false ? "Active" : "Inactive"}
                                            </span>
                                        </div>
                                        <div className="mt-2 space-y-1.5 text-[12px] text-[#7B8497]">
                                            <div>Email: {trainer.user?.email || trainer.email}</div>
                                            <div>Specialization: {trainer.specialization || "Not specified"}</div>
                                        </div>
                                        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#E9EDF4]">
                                            <button
                                                onClick={() => navigate(`/company/admin/trainers/${trainer.userId || trainer.id}/edit`)}
                                                className="flex items-center gap-1 text-[#0475FB] hover:bg-[#EAF3FF] px-3 py-1.5 rounded-lg text-[12px] font-medium transition"
                                            >
                                                <Edit size={14} /> Edit
                                            </button>
                                            <button
                                                onClick={() => handleDelete(trainer.userId || trainer.id)}
