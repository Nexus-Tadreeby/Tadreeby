...
const DEFAULT_NAV_ITEMS = [
  ...
  { label: "Universities", icon: Landmark, path: "/universities" },
...
function Sidebar({
  ...
  return (
    <aside className="group flex h-full w-[72px] shrink-0 flex-col border-r border-gray-100 bg-white transition-all duration-300 hover:w-[248px] max-sm:hover:w-[72px]">
      {/* Logo */}
      <Link to="/" className="flex items-center justify-center px-6 pt-8 pb-6 group-hover:justify-start max-sm:group-hover:justify-center">
        <img
          src={logo}
          alt="Tadreeby Logo"
      ...
      <nav className="mt-2 flex-1 space-y-1">
        {navItems.map(({ label, icon: Icon, path }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex w-full items-center justify-center gap-3 px-4 py-3 text-[14px] transition-all duration-400 group-hover:justify-start max-sm:group-hover:justify-center ${
                isActive
      ...
      <div className="space-y-1 border-t border-gray-100 py-4 px-4">
        {footerItems.map(({ label, icon: Icon, badge, path }) => (
          <NavLink
            key={label}
            to={path}
            className={({ isActive }) =>
              `flex w-full items-center justify-center gap-3 rounded-xl px-4 py-2.5 text-[14px] font-normal text-gray-400 transition-all duration-400 group-hover:justify-start max-sm:group-hover:justify-center hover:bg-gray-50 hover:text-gray-600 ${
      ...
      {/* User profile – now a NavLink to highlight on profile page */}
      <NavLink
        to={profilePath}
        className={({ isActive }) =>
          `flex items-center justify-center gap-2 border-t border-gray-100 px-5 py-4 transition-all duration-400 group-hover:justify-start max-sm:group-hover:justify-center ${
            isActive
              ? "bg-blue-50 text-blue-600" // active profile style
              : "hover:bg-gray-50"
          }`
        }
      >
