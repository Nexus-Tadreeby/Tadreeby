...
function Sidebar({
  discoveryItems = DEFAULT_DISCOVERY,
  ...
  useEffect(() => {
    const saved = window.sessionStorage.getItem("sidebar-expanded");
  ...
  return (
    ...
    >
      ...
      >
        ...
        {expanded && (
          <div className="min-w-0 leading-tight">
            <p className="truncate text-[12px] font-semibold text-[#6F7278]">
              {user.name}
            </p>
            <p className="mt-0.5 text-[9px] font-medium text-[#AEB1B7]">
              {user.role}
            </p>
          </div>
        )}
      ...
      <div className="flex min-h-0 flex-1 flex-col px-3">
        {expanded && (
          <p className="px-2 pb-2 pt-4 text-[8px] font-medium uppercase tracking-wide text-[#B8BAC0]">
            Discovery
          </p>
        )}

        <nav className="space-y-1">{discoveryItems.map(renderNavItem)}</nav>

        {expanded && (
          <p className="px-2 pb-2 pt-7 text-[8px] font-medium uppercase tracking-wide text-[#B8BAC0]">
            Management
          </p>
        )}

        <nav className="space-y-1">{managementItems.map(renderNavItem)}</nav>
        ...
        <div className="mt-auto pb-4">
          ...
          >
            ...
            {expanded && (
              <>
                <span className="text-[13px] font-medium text-[#777A80]">
                  Chat
                </span>

                <div className="ml-auto flex items-center">
                  <div className="flex -space-x-1.5">
                    {(chatAvatars.length ? chatAvatars : ["A", "D", "M"]).map(
                      (avatar, index) => (
                        <div
                          key={index}
      ...
      <div className="shrink-0 border-t border-[#F4F4F5] px-3 pb-2 pt-3">
        {expanded && (
          <p className="px-2 pb-2 text-[8px] font-medium text-[#70737A]">
            Settings
          </p>
        )}
