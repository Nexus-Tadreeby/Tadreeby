...
// // ─── Normalize backend response (both GET and PATCH) ──────────────
// function normalizeProfileResponse(response, previousProfile = {}) {
//   // If response already has a 'data' field, it's the full wrapper
//   const data = response?.data ?? response;
//   const user = data?.user ?? {};
...
//     avatar: user?.profileImage ?? previousProfile.avatar,
//     skills: Array.isArray(data?.skills) ? data.skills : previousProfile.skills,
//     // Keep university as string – will be set by caller
//   };
// }
...
// const DEFAULT_SKILLS = ["React.js", "Tailwind CSS", "JavaScript (ES6+)"];
...
// // ─── University mapping (for fallback) ──────────────────────────────
// const UNIVERSITIES = [
//   { id: 1, name: "An-Najah National University" },
//   { id: 2, name: "Birzeit University" },
//   { id: 3, name: "Bethlehem University" },
...
// // ─── Build initial profile from user (fallback) ──────────────────────
// function buildInitialProfile(user) {
//   const safeUser = user || {};
//   const studentProfile = safeUser.studentProfile || {};
...
// // ================================================================
// // ✅ FIXED InfoIcon – clean SVG, no malformed path
// // ================================================================
// const InfoIcon = () => (
//   <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
//     <path
...
// // ================================================================
// // PROFILE HEADER (with safety check for university string)
// // ================================================================
// const ProfileHeader = ({
//   profile,
//   completion,
...
//   // ✅ Force university to be a string
//   const universityName = typeof profile.university === 'string'
//     ? profile.university
//     : (profile.university?.name || '');
...
//       <div className="absolute left-[7%] right-[7%] top-5 h-0.5 bg-gray-200" />
//       <div
//         className="absolute left-[7%] top-5 h-0.5 bg-[#1677FF] transition-all duration-500"
//         style={{ width: `calc(${fillWidth} * 86%)` }}
//       />
//       {steps.map((step, idx) => {
//         const completed = idx < statusIndex;
...
//           <div key={step.key} className="relative z-10 flex w-1/3 flex-col items-center" role="listitem">
//             <div
//               className={`flex h-10 w-10 items-center justify-center rounded-full border-4 border-white text-xs font-extrabold shadow-sm ${completed
//                 ? "bg-[#1677FF] text-white"
//                 : active
//                   ? "bg-[#1677FF] text-white ring-4 ring-[#EAF3FF]"
...
//                 type="button"
//                 onClick={onCancel}
//                 className="rounded-xl border border-gray-200 px-3.5 py-2 text-xs font-bold text-gray-600 hover:bg-gray-50"
//               >
//                 Cancel
//               </button>
//               <button
//                 type="button"
//                 onClick={onSave}
//                 className="inline-flex items-center gap-1.5 rounded-xl bg-[#1677FF] px-3.5 py-2 text-xs font-bold text-white shadow-sm hover:bg-[#086BEA]"
//               >
//                 <Save className="h-3.5 w-3.5" />
//                 Save Changes
//               </button>
...
//       <div className="space-y-3 p-6">
//         {/* CV upload with simulated progress */}
//         <div
//           role="button"
//           tabIndex={0}
//           onClick={() => !isCvUploading && cvInputRef.current?.click()}
...
//   //       // Ensure university is a string (safety fallback)
...
//   //     } catch (error) {
//   //       console.error("Failed to load profile:", error);
//   //       // Keep the fallback from initial state
//   //     } finally {
//   //       setIsLoadingProfile(false);
//   //     }
//   //   };
...
//       setIsLoadingProfile(true);
//       try {
//         const response = await profileAPI.getProfile(); // { success: true, data: {...} }
//         const normalized = normalizeProfileResponse(response, profile);
//         // Ensure university is a string (if it's an object)
//         if (normalized.university && typeof normalized.university === 'object') {
//           normalized.university = normalized.university.name || '';
//         }
//         setProfile(normalized);
...
//   useEffect(() => {
//     if (location.state?.registrationSuccess) {
//       setToastMessage(location.state.message || "Your account is pending verification.");
...
//     const timer = window.setTimeout(() => setSaveToast(""), 3000);
//     return () => window.clearTimeout(timer);
//   }, []);
...
//     if (file.size > 2 * 1024 * 1024) {
//       showSaveToast("Photo must be under 2MB.");
//       return;
//     }
...
//     try {
//       // Simulate progress (since backend expects base64 via PATCH)
//       await new Promise((resolve) => {
//         let progress = 0;
//         const interval = setInterval(() => {
//           progress += Math.random() * 15 + 5;
...
//       // ✅ Use the response to update the profile
//       setProfile((prev) => ({
//         ...prev,
//         avatar: response?.profileImage || URL.createObjectURL(file),
//       }));
//       showSaveToast("Profile photo updated.");
//     } catch (error) {
//       console.error("Avatar upload error:", error);
//       showSaveToast("Failed to upload photo.");
//     } finally {
//       setIsAvatarUploading(false);
//       setAvatarProgress(0);
//       if (avatarInputRef.current) avatarInputRef.current.value = '';
...
//     if (file.size > 5 * 1024 * 1024) {
//       showSaveToast("CV must be under 5MB.");
//       return;
//     }
...
//         ...prev,
//         hasCv: true,
//         cvUrl: response?.cvFile || file.name,
//       }));
//       showSaveToast("CV uploaded.");
//     } catch (error) {
//       console.error("CV upload error:", error);
//       showSaveToast("Failed to upload CV.");
//     } finally {
//       setIsCvUploading(false);
//       setCvProgress(0);
//       const input = e.target;
...
//   //   if (!draft) return;
//   //   const changes = {};
//   //   const fields = ['recoveryEmail', 'phone', 'gpa'];
//   //   fields.forEach(f => {
//   //     if (draft[f] !== profile[f]) {
//   //       changes[f] = draft[f];
//   //     }
...
//   //     const updatedData = await profileAPI.updateProfile(changes);

//   //     // ✅ Merge the response with current profile
//   //     setProfile((prev) => ({
//   //       ...prev,
//   //       ...updatedData,
//   //       university: typeof updatedData.university === 'object'
//   //         ? updatedData.university?.name || prev.university
//   //         : updatedData.university || prev.university,
//   //     }));
...
//   //   } catch (error) {
//   //     console.error("Update error:", error);
//   //     showSaveToast("Failed to update profile.");
//   //   }
//   // };
//   const handleSave = async () => {
//     if (!draft) return;
...
//     const changes = {};
//     const editableFields = ['recoveryEmail', 'phone', 'gpa']; // only these are editable
//     editableFields.forEach(f => {
//       if (draft[f] !== profile[f]) {
//         changes[f] = draft[f];
//       }
...
//     try {
//       const response = await profileAPI.updateProfile(changes); // returns { success: true, data: {...} }
//       const normalized = normalizeProfileResponse(response, profile);
...
//     } catch (error) {
//       console.error("Add skill error:", error);
//       showSaveToast("Failed to add skill.");
//     }
//   };
...
//         skills: (prev.skills || []).filter((s) => s !== skill),
//       }));
//       showSaveToast("Skill removed.");
//     } catch (error) {
//       console.error("Remove skill error:", error);
//       showSaveToast("Failed to remove skill.");
//     }
//   };
...
//     <div className="flex h-screen w-full overflow-hidden bg-[#F5F7FA] font-sans text-gray-900">
//       {toastMessage && (
//         <div className="fixed right-4 top-4 z-50 max-w-md rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 shadow-xl">
//           <p className="text-base font-extrabold text-emerald-800">Registration Successful</p>
//           <p className="mt-1 text-xs font-medium leading-5 text-emerald-700">{toastMessage}</p>
//         </div>
//       )}

//       {saveToast && (
//         <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full border border-gray-200 bg-white px-5 py-2.5 shadow-lg">
//           <p className="text-xs font-bold text-[#1677FF]">{saveToast}</p>
//         </div>
//       )}
...
//     try {
//       await profileAPI.updateProfile(changes);
//       setProfile(draft);
//       setDraft(null);
//       setIsEditing(false);
//       showSaveToast("Profile updated successfully.");
...
// function buildInitialProfile(user) {
//   return {
//     firstName: user?.firstName || "Afnan",
//     lastName: user?.lastName || "Kullab",
//     email: user?.email || "afnankullab3@gmail.com",
//     phone: user?.phone || "",
//     studentNumber: user?.studentNumber || "",
//     major: user?.major || "Software Engineering",
//     university: user?.university || "Palestine University",
//     recoveryEmail: "",
//     gpa: "",
//     company: "",
//     avatar: "",
...
//   const required = checks.filter((c) => !c.optional);
//   const doneCount = required.filter((c) => c.done).length;
//   const percentage = Math.round((doneCount / required.length) * 100);
...
//     })),
//     percentage,
//     nextAction: checks.find((c) => !c.done && !c.optional),
//   };
// }
...
// // ------------------------------------------------------------
// // Profile header — inspired by the reference layout
// // ------------------------------------------------------------
// const ProfileHeader = ({ profile, completion, onAvatarUpload, fileInputRef, onTabChange }) => {
...
//     <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-[0_4px_20px_rgba(20,35,60,0.06)]">
//       {/* Banner */}
//       <div className="relative h-[150px] overflow-hidden bg-gradient-to-r from-[#1677FF] via-[#3F8CFF] to-[#FFAD4E] sm:h-[172px]">
//         <div className="absolute -right-10 -top-20 h-56 w-56 rounded-full bg-white/15 blur-3xl" />
//         <div className="absolute -bottom-24 left-[38%] h-56 w-56 rounded-full bg-[#FFAD4E]/30 blur-3xl" />
...
//               </p>
//               <div className="mt-2 flex flex-wrap items-center gap-2">
//                 <span className="inline-flex items-center gap-1 rounded-full bg-[#FFF4E6] px-2.5 py-1 text-[10px] font-bold text-[#C76A0B]">
//                   <AlertCircle className="h-3 w-3" />
...
//           <div className="px-4 py-3">
//             <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">University</p>
//             <p className="mt-1 truncate text-xs font-bold text-gray-800">{profile.university || "—"}</p>
//           </div>
//           <div className="border-t border-gray-100 px-4 py-3 sm:border-t-0">
//             <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">GPA</p>
//             <p className="mt-1 truncate text-xs font-bold text-gray-800">{profile.gpa || "Not added"}</p>
...
//             <div className="mt-1 flex items-center gap-2">
//               <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-200">
//                 <div className="h-full rounded-full bg-[#1677FF]" style={{ width: `${completion.percentage}%` }} />
//               </div>
//               <span className="text-xs font-extrabold text-[#1677FF]">{completion.percentage}%</span>
//             </div>
//           </div>
//         </div>
//       </div>
...
//     <div className="border-b border-gray-100 bg-gray-50/60 px-6 py-4">
//       <div className="flex items-start gap-3">
//         <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#FFF4E6] text-[#C76A0B]">
//           <InfoIcon />
//         </div>
//         <div>
//           <h2 className="text-sm font-extrabold text-gray-900">Account Verification</h2>
//           <p className="mt-0.5 text-xs font-medium text-gray-500">
//             Your profile is under review by university administration
//           </p>
//         </div>
...
// }) => {
//   const fields = [
//     ["First Name", profile.firstName, true],
//     ["Last Name", profile.lastName, true],
//     ["Major", profile.major, false],
//     ["University", profile.university, true],
//     ["Email", profile.email, true],
//     ["Recovery Email", profile.recoveryEmail, false],
//     ["Phone", profile.phone, false],
//     ["Student Number", profile.studentNumber, true],
//     ["GPA (optional)", profile.gpa, false],
//     ["Company", profile.company, false],
//   ];
...
//         description={
//           isEditing
//             ? "Edit editable fields. Name and email are locked after registration."
//             : "Your account details visible to supervisors and admins."
//         }
//         action={
//           !isEditing ? (
...
//         {isEditing ? (
//           <>
//             <FieldInput label="First Name" value={profile.firstName} locked onChange={() => { }} />
//             <FieldInput label="Last Name" value={profile.lastName} locked onChange={() => { }} />
//             <FieldInput label="Major" value={profile.major} onChange={(e) => onChange("major", e.target.value)} placeholder="Your specialization" />
//             <FieldInput label="University" value={profile.university} locked onChange={() => { }} />
//             <FieldInput label="Email" value={profile.email} locked icon={<Mail className="h-4 w-4" />} onChange={() => { }} />
//             <FieldInput
//               label="Recovery Email"
//               type="email"
...
//               icon={<Mail className="h-4 w-4" />}
//             />
//             <FieldInput label="Phone" value={profile.phone} onChange={(e) => onChange("phone", e.target.value)} placeholder="+970 5XX XXX XXX" icon={<Phone className="h-4 w-4" />} />
//             <FieldInput label="Student Number" value={profile.studentNumber} locked icon={<Hash className="h-4 w-4" />} onChange={() => { }} />
//             <FieldInput label="GPA (optional)" value={profile.gpa} onChange={(e) => onChange("gpa", e.target.value)} placeholder="e.g. 3.75" />
//             <FieldInput label="Company" value={profile.company} onChange={(e) => onChange("company", e.target.value)} placeholder="Assigned after internship placement" />
//           </>
//         ) : (
//           <>
//             <FieldDisplay label="First Name" value={profile.firstName} locked />
//             <FieldDisplay label="Last Name" value={profile.lastName} locked />
//             <FieldDisplay label="Major" value={profile.major} />
//             <FieldDisplay label="University" value={profile.university} locked />
//             <FieldDisplay label="Email" value={profile.email} icon={<Mail className="mr-2 h-4 w-4 shrink-0 text-[#1677FF]" />} locked />
//             <FieldDisplay
//               label="Recovery Email"
//               value={profile.recoveryEmail}
//               placeholder="Add a recovery email for account security"
...
//               highlight={highlightField === "recoveryEmail"}
//             />
//             <FieldDisplay label="Phone" value={profile.phone} placeholder="Add your phone number" icon={<Phone className="mr-2 h-4 w-4 shrink-0 text-gray-400" />} />
//             <FieldDisplay label="Student Number" value={profile.studentNumber} placeholder="From registration" icon={<Hash className="mr-2 h-4 w-4 shrink-0 text-gray-400" />} locked />
//             <FieldDisplay label="GPA (optional)" value={profile.gpa} placeholder="Enter your GPA" />
//             <FieldDisplay label="Company" value={profile.company} placeholder="You are not enrolled yet" />
//           </>
//         )}
//       </div>
//     </SectionCard>
...
// // Documents + skills
// // ------------------------------------------------------------
// const DocumentsCard = ({ profile, onCvUpload }) => {
//   const cvInputRef = useRef(null);
...
//           className={`group flex cursor-pointer items-center justify-between rounded-xl border p-4 transition ${profile.hasCv
//               ? "border-[#1677FF]/20 bg-[#EAF3FF]/60"
//               : "border-dashed border-gray-200 bg-gray-50/50 hover:border-[#1677FF]/40 hover:bg-[#EAF3FF]/40"
//             }`}
//         >
//           <div className="flex min-w-0 items-center gap-3">
//             <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${profile.hasCv ? "bg-[#1677FF]/10" : "bg-gray-100"}`}>
//               <FileText className={`h-5 w-5 ${profile.hasCv ? "text-[#1677FF]" : "text-gray-400"}`} />
//             </div>
//             <div className="min-w-0">
//               <span className="text-xs font-bold text-gray-800">CV / Resume</span>
//               <p className="truncate text-[10px] font-medium text-gray-500">
//                 {profile.hasCv ? "resume.pdf · Uploaded" : "PDF or DOC · Drag or click to upload"}
//               </p>
//             </div>
//           </div>
//           {profile.hasCv ? <Download className="h-4 w-4 text-[#1677FF]" /> : <Upload className="h-4 w-4 text-gray-400" />}
...
//             <div className="min-w-0">
//               <span className="text-xs font-medium text-gray-700">Verification Document</span>
//               <p className="truncate text-[10px] font-medium text-gray-500">Submitted during registration</p>
//             </div>
//           </div>
//           <span className="shrink-0 rounded-full border border-[#FFAD4E]/30 bg-[#FFF4E6] px-2.5 py-1 text-[10px] font-bold text-[#C76A0B]">
//             Under Review
//           </span>
//         </div>
//       </div>
...
//     if (e.target.files?.[0]) {
//       setProfile((prev) => ({ ...prev, hasCv: true }));
//       showSaveToast("CV uploaded successfully.");
//     }
//   };
...
//                 type="search"
//                 placeholder="Search internships or tasks..."
//                 aria-label="Search internships or tasks"
//                 className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-11 pr-4 text-xs text-gray-700 shadow-[0_1px_5px_rgba(20,35,60,0.03)] transition focus:border-[#1677FF] focus:outline-none focus:ring-4 focus:ring-[#1677FF]/10"
//               />
//             </div>
...
//                 <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
//                   <div ref={documentsRef}>
//                     <DocumentsCard profile={profile} onCvUpload={handleCvUpload} />
//                   </div>
//                   <div ref={skillsRef}>
//                     <SkillsCard
//                       skills={profile.skills}
...
//                 <div className="space-y-3 p-5">
//                   <div className="flex items-center gap-3">
//                     <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#FFF4E6] text-[#C76A0B]">
//                       <ShieldCheck className="h-4 w-4" />
//                     </div>
//                     <div>
//                       <p className="text-xs font-bold text-gray-800">Under Review</p>
//                       <p className="text-[10px] text-gray-500">University administration</p>
//                     </div>
//                   </div>
//                   <div className="flex items-center gap-3">
//                     <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#EAF3FF] text-[#1677FF]">
