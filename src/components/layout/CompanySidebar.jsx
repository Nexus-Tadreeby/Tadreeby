...
const DEFAULT_FOOTER_ITEMS = [
    { label: "Settings", icon: Settings, path: "/company/admin/settings" },
];

function CompanySidebar({
    navItems = DEFAULT_NAV_ITEMS,
    footerItems = DEFAULT_FOOTER_ITEMS,
    user = { name: "Company Admin", role: "Admin", avatar: "" },
    profilePath = "/company/admin/profile",
    chatPath = "/company/admin/chats",
    ...
    useEffect(() => {
        const saved = sessionStorage.getItem("company-sidebar-expanded");
        if (saved !== null) setExpanded(saved === "true");
    }, []);

    useEffect(() => {
        sessionStorage.setItem("company-sidebar-expanded", expanded ? "true" : "false");
    }, [expanded]);
    ...
    return (
        ...
        >
            ...
            <div className="relative flex h-[70px] shrink-0 items-center justify-center border-b border-[#F4F4F5]">
                <Link to="/company/admin/dashboard" aria-label="Company Dashboard">
                    <svg width="47" height="30" viewBox="0 0 47 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                        {/* ضع هنا SVG الشعار الخاص بك، أو استخدم نفس الشعار الموجود في Sidebar الأصلي */}
                            ...
                            <>
                                ...
                                <div className="ml-auto flex items-center">
                                    <div className="flex -space-x-1.5">
                                        {(chatAvatars.length ? chatAvatars : ["C", "A"]).map((avatar, index) => (
                                            <div
                                                key={index}
